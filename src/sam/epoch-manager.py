...
def main(data):
    ...
    # One-time publication of all data partitions
        ...
        def the_facet_body():
            ...
            def k(local_model):
                ...
                if len(collected_results) == len(train_partitions):
                    ...
                    turn.log.info(
                        f"Epoch: {current_epoch} / {cfg.N_EPOCHS} | "
                        f"Final Model Evaluation: {updated_global_model.evaluate(test_set)} / {len(test_set)}"
                    )
                    turn.stop(
                        facet       = epoch_facet,
                        continuation = lambda: run_epoch(
                            updated_global_model,
                            current_epoch + 1
                        )
                    )

            # Publish the model + partition ID for each worker
